...
source("utils/R/env.R")
library(tidyverse)

# Parameters
samples_path = "data/samples.tsv"
cellranger_bcr_contigs_dir = "data/contigs/ig/filtered"
cellranger_tcr_contigs_dir = "data/contigs/tr/filtered"
cellranger_contigs_prefix = "filtered"
seq_run_ids = c("SLE_1A_total", "sle_batch2_pool1")

out_dir = create_dir("results/step0")
...
# Create samples.txt with locations of cellranger VDJ contigs and fasta

samples_mdata_lst <- lapply(seq_run_ids, function(seq_run_id) {
  
  cbind.data.frame(
    sample_id = seq_run_id,
    sample_output_id = seq_run_id,
    BCR.location = file.path(
      cellranger_bcr_contigs_dir,
      seq_run_id,
      paste0(cellranger_contigs_prefix, "_contig_annotations.csv")
    ),
    TCR.location = file.path(
      cellranger_tcr_contigs_dir,
      seq_run_id,
      paste0(cellranger_contigs_prefix, "_contig_annotations.csv")
    ),
    Overall_sample_group = seq_run_id,
    Site = "blood",
    batch = seq_run_id,
    out_dir = out_dir,
    out_dir_raw = paste0(out_dir, "/")
  )
  
})
samples_mdta_df <- do.call("rbind", samples_mdata_lst)
write_tsv(samples_mdta_df, file.path(out_dir, "metadata.tsv"))
...
if(a==1){
  ...
  #Remove empty rows?
  p=p[which(p[,1]!=""),]
  sample_id = p$sample_id #as.character(p[,"Sample_Name"])
  sample_output_id = p$sample_output_id #as.character(p[,"Sample_Name"])
  BCR.location = p$BCR.location #as.character(p[,"Location_of_BCR"])
  #BCR.location = gsub("all_contig","filtered_contig",BCR.location)
  TCR.location = p$TCR.location #as.character(p[,"Location_of_TCR"])
  #TCR.location = gsub("all_contig","filtered_contig",TCR.location)
  Overall_sample_group = p$Overall_sample_group #as.character(p[,"Patient"])
  Site = p$Site #as.character(p[,"Sample_type"])
  batch = p$batch #"PDAC_LT1"
  out_dir = p$out_dir #"/well/immune-rep/shared/10X_GENOMICS/PDAC_LT_SURVIVORS/VDJ/"
  out_dir_raw = p$out_dir_raw #"/well/immune-rep/shared/10X_GENOMICS/PDAC_LT_SURVIVORS/VDJ/"
  
}

# Expected to be just 1 element and sample for all samples in functions because a==1
out_dir_raw = out_dir_raw[1]
out_dir = paste0(out_dir[1], "/")

BCRs = 1:length(BCR.location)
TCRs = 1:length(TCR.location)
...
Move_files_to_working_directory<-function(TCR.location, BCR.location, sample_id, out_dir_raw){
  for(c in 1:length(BCR.location)){
  #for(c in BCRs){
    if(is.na(BCR.location[c])==F){
      sample = sample_id[c]
      fasta_file = gsub("_annotations.csv",".fasta",BCR.location[c] )
  ...
  for(c in 1:length(TCR.location)){
  #for(c in TCRs){
    if(is.na(TCR.location[c])==F){
      sample = sample_id[c]
      fasta_file = gsub("_annotations.csv",".fasta",TCR.location[c] )
...
# Liezel - IMGT annotations mapped back to per sample file
...
Map_annotations_by_sample<-function(dir_IMGT_TCR, dir_IMGT_BCR, TCRs, BCRs,  sample_id){
  ...
  for(f in c(1:length(files))){
    ...
    ### split by sample # Liezel - Creating the fasta split by sample, not filtering done
    used = NULL
    found = NULL
    library(ape)
    for(c in BCRs){
...
# Liezel: Below code to generate per cell information
...
Group_sequence_files<-function(combined_sequence_file, 
                               cell_info_files, 
                               sample_fastas,
                               id,
                               samples_group,
                               amino_acid_fastas,
                               output_file_prefix,cluster_file){
  library(ape)
  library(igraph)
  ...
  # Liezel: lower sequence similarity so i can have enlarged clusters
  command= concat(c(cd_hit_directory,"cd-hit -i ",combined_sequence_file," -o ",tmp_file," -c 0.95 -d 180 -T 10  -M 0 -AL 40 "))
  print (command)
  system(command)
  ...
  seq_id = NULL
  cluster_match = NULL
  # Liezel: Collect cluster id for each uniq sequence
  ...
  # Liezel: Cluster membership of each unique sequence (in uniqueID)
  ...
  # Liezel - kmer-count based distance
  ...
  # Liezel: Get sliding (1-nt at a time) k-mer of size 3
  ...
  if(length(enlarged_clusters)>0){
    for(c in c(1:length(enlarged_clusters))){
      ...
      # Liezel - Generate kmercounts_mat with rows equal to number of unique sequences in a cluster
      # and columns equal to number of all_kmers in sequences, then actual values is the count of kmer per sequence
      ...
      # Liezel - so we are very stringent as we take max distance,
      # largest absolute difference between any corresponding elements of the two vectors (or rows)
      d = as.matrix(dist(kmercounts_mat, method = "maximum"))
      # Liezel: (Ask) This bit here only assumes that there are 2 sequences though
      # Liezel: why not max(d)?
      if(max(d[1,2])<=1){ # Liezel - distance threshold for setting edges
        ...
        # Liezel - within a cluster of sequence, if there are some that are very different
        # from the rest, represent their edge accurately to reflect that
        d[which(d>1)] = 100
        # Liezel:
        # Warning: `graph.adjacency()` was deprecated in igraph 2.0.0.
        #ℹ Please use `graph_from_adjacency_matrix()` instead.
        # diag = TRUE by default so it is accounted for in the calculation but 
        # dist() does put 0s in diagonal equivalent to diag = FALSE
  ...
  # Liezel - Warning: `graph.empty()` was deprecated in igraph 2.1.0.
  # ℹ Please use `make_empty_graph()` instead.
  g <- graph.empty(n=0, directed=T)
  unique_seq_ids = names(unique_seqs)
  # Liezel - Warning: `add.vertices()` was deprecated in igraph 2.0.0.
  ...
  # Liezel - "components(g): This function computes the connected components of the graph g. A connected component is a subset of the graph where there is a path between every pair of nodes (vertices) in that subset. In simpler terms, it identifies disconnected subgraphs within the overall graph. If the graph is already fully connected, then there is just one component."
...
for(o in c(1:length(Overall_sample_groups))){
  ...
  if(length(samples_group)>0){
    ...
    for(ch in c(1:length(chains))){
      ...
      # Liezel - This sample_fastas not really used in Group_sequence_files()
      # And cellranger i think already trims barcodes in VDJ sequences?
      ...
      # Liezel - comment out as sample_fastas not really used in Group_sequence_files()
      #info = file.info(sample_fastas)
      #w = which(info$size != 0)
      #sample_fastas = sample_fastas[w]
      #amino_acid_fastas = amino_acid_fastas[w]
      
      #Liezel - not sure why indexing with w needed as this is done per sample
      #cell_info_files = paste0(concat(c(out_dir_raw,"Cell_annotation_")), samples_group[w],"_IG.txt" )
      cell_info_files = paste0(concat(c(out_dir_raw,"Cell_annotation_")), samples_group,"_IG.txt" )
...
for(o in c(1:length(Overall_sample_groups))){
  ...
  if(length(samples_group)>0){
    ...
    for(ch in c(1:length(chains))){
      ...
      cell_info_files = paste0(concat(c(out_dir_raw,"Cell_annotation_")), samples_group,"_TCR.txt" )
      id = concat(c(grouped_sample_id ,"_",chain))
