...
Get_TCR_information<-function(input_directory_groups){
  ...
  p <- cbind(
    p,
    "Full.annotation" = p[ , "Level.1"],
    "Very.broad.annotation" = p[ , "Level.2"],
    "Broad.annotation" = p[ , "Level.2"]
  )
...
BCR_clonal_overlap_per_site<-function(VDJ_list_BCR, output_directory,groups_PCA){
  ...
  # Liezel: Number of cells with clone id per cell type per sample (repertoire)
  # Liezel: And covert those numbers to proportion of cell types per sample
  ...
  # Liezel: Sum cells with receptor (clone != "-") per sample
  totals = rowSums(m_cell_types)
  
  ############# get overlap between compartments
  c1 = NULL
  c2 = NULL
  # Liezel: Get all possible pairs of cell_types
  ...
  # Liezel: Per pair of cell types, calculate overlap
  for(s in c(1:length(samples))){
    print (s)
    ...
    for(i in c(1:length(c1))){
      ...
      # Liezel - intersect takes union of clones so not accounting for actual number of cells per shared clones
  ...
  for(s in c(1:length(samples))){
    ...
    # Liezel: Num cells per clone per celltype
    t = table(clone[wa], cell_type[wa])
    # Liezel: Get clones with cells/members in more than 1 cell type because
    # there are only the clones that you can use to make graph (w is list of those clones)
    w = which(apply(t, 1, function(x){length(which(x!=0))})>1)
    if(length(w)>0){
      # print(r)
      t=rbind(t[w,])
      # Liezel: per cell type pair, count how many clones share that two cell types
      # So overlap is the number of unique clones that share that cell type not considering actual number of cells in clone
    ...
    # Liezel: Normalise by total number of overlaps between all pairs of cell types
  ...
  V(g)$color = "grey"
    ...
    w=2.7
    # Added by Liezel
    fileout1=concat(c(output_directory,"",analysis,"_network_blood_", batch,".pdf"))
    pdf(file=fileout1, height=w*1, width=w*3)
    par(mfrow= c(1,3), mar = c(1,1,3,1))
...
TCR_clonal_overlap_per_site<-function(VDJ_list_TCR, output_directory,CD48, groups_PCA){
  ...
  for(cd48 in c(1:length(CD48))){
    ...
    # Table of links between cell types per sample, 0 means no link or shared clones
    overlapping_per_link = matrix(data = 0,nrow = length(samples), ncol = length(links), dimnames = c(list(samples), list(links)))
    for(s in c(1:length(samples))){
      mat = list_overlapping_per_sample[[s]]
      for(c1 in c(1:length(cell_types))){
...
BCR_immunosurveillance<-function(VDJ_list_BCR, output_directory, batch, groups_PCA){
  ...
  for(s in c(1:length(Patients))){
    # Liezel: Biopsy cells of this patient
    w1 = intersect(which(Patient == Patients[s]), which(Sample.Type== Sample.Types[1]))
    # Liezel: Blood cells of this patient
    w2 = intersect(which(Patient == Patients[s]), which(Sample.Type== Sample.Types[2]))
    # Liezel: Cells with receptor not "-"
    w1 = intersect(w1, which(id %in% w_clone))
    w2 = intersect(w2, which(id %in% w_clone))
    # Liezel: Unique shared clones (not accounting number of cells)
    shared_clones = intersect(clone[id[w1]] , clone[id[w2]])
    cl1 = clone[id[w1]]
    cl2 = clone[id[w2]]
    # Liezel: Two sets of cells to compare
    cls = c(list(cl1),list(cl2)) ## shared clones
    # Liezel: If both sets have greater than threshold, subsample to threshold cells repeats times
    # Liezel: Sampling without replacement
    if(min(c(length(cl1), length(cl2)))> threshold){
      ...
      for(r in c(1:repeats)){
        ...
        # Liezel: Number of shared unique clones after making number of cells equal
        o = intersect(rand1, rand2)
        overlap_clones = c(overlap_clones, length(o))
        # Proportion of cells in shared clones in set 1
        overlap1 = c(overlap1, length(which(rand1 %in% o))*100/length(rand1))
        # Proportion of cells in shared clones in set 2
