...
# ** Double check that this uses sliding window, if not use scRepertoire::percentKmer() **
...
kmer_usage <- function(
  ...
  # .coding = FALSE always because the filtering for productive (coding) sequences
  # uses immunarch-specific columns
  kmer_counts_df <- immunarch::getKmers(
    df_list, motif_length, sequence_type, .coding = FALSE#, ...
  ) %>% 
